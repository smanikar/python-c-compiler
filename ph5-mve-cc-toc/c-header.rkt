...
(define-struct closure (label env))
  ...
  (syntax-rules ()
    [(_ f args ...)
     (let ([$f f])
       (cond
         [(closure? $f)   
          ((closure-label $f) (closure-env $f) args ...)]
         
         [(procedure? $f)
          ($f args ...)]
         
         [else 
          (error (format "cannot apply non-closure: ~s~nto:~s~n" $f (list args ...)))]))]))

(define-syntax (env-ref stx)
  (syntax-case stx ()
    [(_ eid env field)
     (with-syntax ([extractor (datum->syntax #'eid 
                                             (string->symbol
                                              (string-append
                                               (symbol->string (syntax->datum #'eid))
                                               "-"
...
;; Technically, these should pass through a table to keep scopes separate:
